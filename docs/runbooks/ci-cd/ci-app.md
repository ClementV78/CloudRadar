...
Code quality trend and quality-gate status are validated in a dedicated workflow:
- Workflow: `.github/workflows/sonarcloud.yml`
- Trigger: `workflow_dispatch`, `pull_request -> main`, and `push -> main` (app changes)
- Input reports:
  - Frontend lcov: `src/frontend/coverage/lcov.info`
  - Java JaCoCo XML:
...
- `SONAR_TOKEN is not configured`
  - add `SONAR_TOKEN` in repository secrets before using the workflow

Detailed setup and troubleshooting:
- `docs/runbooks/ci-cd/sonarcloud.md`
...
3. **frontend** (React + Vite + Nginx) → `src/frontend/Dockerfile`
4. **dashboard** (Java 17 + Spring Boot) → `src/dashboard/Dockerfile`
5. **health** (Python) → `src/health/Dockerfile`
6. **admin-scale** (Python) → `src/admin-scale/Dockerfile`
