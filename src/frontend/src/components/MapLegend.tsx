...
export function MapLegend(): JSX.Element {
  ...
  return (
    <div className={`map-legend-shell${isOpen ? ' is-open' : ''}`}>
      <button
        type="button"
      ...
      <aside id={legendPanelId} className="map-legend glass-panel" aria-label="Aircraft legend" hidden={!isOpen}>
        <div className="map-legend-head">
          <h3>Map legend</h3>
          <button type="button" className="map-legend-close" aria-label="Close legend" onClick={() => setIsOpen(false)}>
            Close
          </button>
        </div>
