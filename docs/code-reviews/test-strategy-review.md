...
Current repository status after #490/#491/#492/#493/#506/#507/#514 merges:

| Service | Language | Source files | Tests | Type | Test framework |
|---|---|---|---|---|---|
| **dashboard** | Java/Spring Boot 3.5.11 | 29 | 7 test classes / 37 tests | `@WebMvcTest` + unit tests + `@SpringBootTest` + Redis Testcontainers integration | `spring-boot-starter-test` |
| **ingester** | Java/Spring Boot 3.5.11 | 13 | 3 test classes / 3 tests | `@SpringBootTest` + mapping/parsing unit + Redis Testcontainers integration | `spring-boot-starter-test` |
| **processor** | Java/Spring Boot 3.5.11 | 8 | 3 test classes / 4 tests | `@SpringBootTest` + JSON contract unit + Redis Testcontainers integration | `spring-boot-starter-test` |
...
- `sonarcloud.yml`: quality-gate analysis on PR/main with frontend lcov + Java JaCoCo coverage ingestion (implemented via #507 + #514)
- Current baseline now covers Java services + frontend smoke, with Redis data-path integration merged (#492).
...
| **Integration** (data-path) | Redis Testcontainers | ✅ Implemented (#492) | `src/*/src/test/` | — |
| **HTTP contract** | MockWebServer / JSON payload | ❌ Missing | — | ~3h |
| **E2E smoke** (infra) | Edge path checks + `/api/flights` JSON contract | ✅ Implemented (#493) | `ci-infra.yml` | — |
...
| **Quality gate / trends** | SonarCloud | ✅ Implemented (#507 + #514, frontend + Java scope) | `.github/workflows/sonarcloud.yml` | — |
...
| **Performance baseline** | k6 / Artillery | ✅ Implemented (#493, nightly workflow) | `.github/workflows/k6-nightly-baseline.yml` | — |
...
| **App-level smoke still narrow** (`/api/flights` JSON assertion only) | Baseline exists but deeper scenario coverage remains to add |
...
- [x] one application-level smoke check added in CI (`/api/flights` -> 200 + JSON contract) (issue #493 merged)

**Phase 2 — Integration:**
- [x] Redis Testcontainers tests added across all three Java services (issue #492 merged)
...
- [x] frontend Vitest UI smoke tests added (`App` + map empty/non-empty) (issue #507 merged)
- [x] SonarCloud workflow and project config added (frontend lcov + Java JaCoCo ingestion + quality gate) (issues #507 + #514 merged)

**Phase 4 — Excellence (optional, strong interview impact):**
- [x] k6 baseline script/workflow with thresholds added (issue #493 merged)
