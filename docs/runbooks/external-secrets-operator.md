...
> Comprehensive guide for deploying, configuring, and troubleshooting External Secrets Operator in CloudRadar.

## Overview

External Secrets Operator syncs secrets from AWS SSM Parameter Store into Kubernetes Secrets automatically.

**Architecture:**
```
SSM Parameter Store ← source of truth
    ↓ (ESO polls)
SecretStore ← "How to access SSM"
    ↓
ExternalSecret ← "Which param → which K8s Secret"
    ↓
K8s Secret ← auto-created and refreshed
    ↓
Pods ← mount or env var
```

See [ADR-0016](../architecture/decisions/ADR-0016-2026-01-29-external-secrets-operator.md) for design decisions.
...
- ✅ Secrets in AWS SSM Parameter Store at `/cloudradar/*` (see [aws-account-bootstrap.md](./aws-account-bootstrap.md))
- ✅ IAM policy added to k3s_nodes role (terraform apply in progress)
